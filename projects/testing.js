...
this.sob = Number(this.expression[n]);
...
this.stop = true;
    ...
    this.expression[n] = this.fatorado;
  }}
...
this.ad = this.expression[Number(y) - 1];
    for(this.h = 0;this.h < Number(this.expression[Number(y) + 1]) - 1;this.h++) {
      this.expression[Number(y) - 1] += this.ad;
    }
    this.expression.splice(Number(y) + 1, 1);
    this.expression.splice(y, 1);
  } }
...
this.segs[qual].mons.push(this.m)      
 ...
 for(this.nl = 0;this.nl < this.segs.length;this.nl++) {
     for(this.n = 0;this.n < this.segs.length;this.n++) {
         if(this.n != this.nl) {
         if(String(this.segs[n].car).search(String(this.segs[nl].car)) != -1) {
...
this.uy = ''
                    ...
                    for(this.xarope in this.right) {
                     ...
                     }else if(this.right[xarope] == '*' && String(right[Number(xarope) + 1]).search('[0-9]') == -1){ // 2*x
                     }else if(this.right[xarope] == '*' && String(right[Number(xarope) - 1]).search('[0-9]') == -1){ // x*2
                     }else{
...
this.podeser[belief].position.push(String(java))
                }
...
if(String(list[n]).search('[\\-\\+]') != -1 && this.this.n != 0) {
...
this.partlet = ''
  for(this.y in this.particles[huh].numero) {
   if(String(particles[huh].numero[y]).search('[a-z]') != -1) {
     this.partlet+= String(particles[huh].numero[y])
   }}
...
console.log(engine.FATORAR(ep)[0])