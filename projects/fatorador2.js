...
*/
  ...
  for (i in roller) {
    console.log('repetidos:',roller[i].opl,'way:', roller[i].rept,'posições:', roller[i].position)
    roll.push({repetidos:roller[i].opl, way:roller[i].rept, posições:roller[i].position})
      }

  INDIRETARROLL();
 
  for (echo in roll) {
    console.log(roll[echo].repetidos, roll[echo].posições, roll[echo].way)
  }
  ...
  for (eep in miss) {
    for (quad in miss) {
      if (quad != eep) {
        ...
        for (i in roller) {
            console.log(roller[i].opl, roller[i].position)
              }
