...
console.log(FATORAR2(FATORE('x^2 - 3x - 2x + 6')))
function VAI(str) {
  comofica = [];
  ground_2 = "";
  for (bye in str) {
...
function FATORAR2(expression_2) {
  ...
  while (java < miss_2.length && deucerto == false) {
    ...
    if (podeser.length > 0) {
      for (cold in podeser) {
        if (
          podeser[cold].position.length == quantosfatores_2 &&
          podeser[cold].opl.length == quantasposições_2 &&
          ...
          for (yehaa in podeser[cold].position) {
            ...
            if (miss_2[podeser[cold].position[yehaa]].dividido[0] == "-") {
              ...
              for (ohjesus in miss_2[podeser[cold].position[yehaa]].dividido) {
                if (
                  miss_2[podeser[cold].position[yehaa]].dividido[ohjesus] != "-"
                ) {
  ...
  if (qualexp != "" && parar == false) {
    ...
    for (phy in qualexp) {
      if (qualexp[phy].search("[0-9]") != -1) {
        if (ground_2.search("[0-9]") != -1) {
          ground_2 += qualexp[phy];
        } else if (ground_2.length > 0) {
          expression_2.push(ground_2);
          ground_2 = qualexp[phy];
        } else {
          ground_2 = qualexp[phy];
        }
    ...
    //resultado = FATORAR(expression_2,0);
    //return resultado;
    return expression_2
  }
}
