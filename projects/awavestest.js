...
for (let i = 0; i < contsnumber; i++){
   if (i < 0) {
      ...
      intlimits.push({xinf: i*radius*2, xsup: i*radius*2 + 2*radius, y: 200*Math.sin(i*radius*2),vel:0})
   }
}
...
for (let i = 0; i < contsnumber; i++){
   ...
   gamma = -0.09
   intlimits[i].y += Math.exp(gamma*timer)*Math.sin(k) * ans[n-1] * Math.cos(wn * timer) //+ Math.sin(k) * bns[n - 1] * Math.sin(wn * timer);
